...
public class AppBook {
	...
	public static void retrieveChangeDelete() {
		...
		//No need to persist since book already in the persistence  context,  it will just update
		//session.persist(rename);

		//Delete a book different from the just updated one
		//Using load since i want to hit the database only once
	   Book toDelete = (Book)session.load(Book.class,2L);
